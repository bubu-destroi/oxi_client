 ...
 */
  ...
  const getSingleWorkshop = async (id) =>{
    try {
      ...
      setSelectedTeachers(response.data.teachers.map((teacher) => teacher._id));
      setMinimum_age(response.data.minimum_age)
      setMaximum_age(response.data.maximum_age)
      setMaxParticipants(response.data.maxParticipants)
      setMinParticipants(response.data.minParticipants)

    } catch (error) {
      console.log('error fetching single workshop', error)
    }
  } 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/workshops/${workshopID}`,
        updatedWorkshop
      );
      console.log('success editing workshop')
      setTitle('');
      setDescription('');
      setImage('');
      setDuration('');
      ...
      console.log(error.data, 'error editing workshop');
    }
  };
